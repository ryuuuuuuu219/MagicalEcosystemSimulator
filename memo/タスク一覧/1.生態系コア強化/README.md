# 1.生態系コア強化

参照元: `git show f5a7322:memo/ROADMAP.md`

## 目的

- wander の見え方を改善し、地形や環境差に応じた行動差を増やす。

## 現状（git参照時点）

- 餌・脅威・境界・wander の加重合成は実装済み。
- 地形法線補正、evasion、zigzag、追跡停止閾値は一部実装済み。
- threat map の主移動ロジック統合は未完。
- heat field を安全行動へ使うロジックは未実装。

## 作業ファイル

1. `1.行動意思決定仕様の明文化.txt`
- threat map / heat field の統合方針を仕様として確定する。

2. `2.行動パラメータ設計の更新.txt`
- 地形、遮蔽、記憶の設計値を実装項目に対応づける。

3. `3.行動ベクトル統合.txt`
- threat / heat を既存の移動合成に反映する。

4. `4.場の評価・可視化連携.txt`
- threat / heat の場データを行動判断に接続する。

5. `5.地形・境界条件との整合.txt`
- 境界回避、地形依存移動との整合を取る。

各テキストファイルに、作業単位・対象ファイル・完了条件を分解して記載する。

## 完了条件

- 地形条件の違いで移動パターンが目視で判別できる。
- 追跡開始・継続・放棄の遷移根拠をログで確認できる。

## 進捗マトリックス（2026-04-07時点）

列は着手順に並べる。

| ファイル | 1.仕様整理・設計 | 2.実装に落とす前準備 | 3.実コードへの統合 | 4.検証・可視化・地形整合 |
| --- | --- | --- | --- | --- |
| README.md | ◎ | ○ | △ | △ |
| 1.行動意思決定仕様の明文化.txt | ◎ | ○ | △ | △ |
| 設定：移動についてのメモ.txt | ◎ | ◎ | ○ | △ |
| 設定：field設計まとめ.txt | ◎ | ◎ | △ | ○ |
| 熱量の演算シミュレーション.txt | ◎ | ○ | △ | ○ |
| 設定：ゲノム構造（基礎拡張）.txt | ○ | ◎ | △ | - |
| 2.行動パラメータ設計の更新.txt | ○ | ◎ | △ | △ |
| 3.行動ベクトル統合.txt | ○ | ○ | ◎ | △ |
| 4.場の評価・可視化連携.txt | ○ | ○ | ○ | ◎ |
| 5.地形・境界条件との整合.txt | △ | ○ | ○ | ◎ |
| 魔素の演算シミュレーション.txt | ○ | △ | - | △ |

凡例
- ◎: その段階の主資料
- ○: 強く関係
- △: 補助的に関係
- -: 関連薄め

## 着手順と現状整理（2026-04-07時点）

| フェーズ | 優先 | ファイル | 主な役割 | 状態 |
| --- | --- | --- | --- | --- |
| 1.仕様整理・設計 | 1 | README.md | 全体目的、現状、完了条件の基準 | 進行中 |
| 1.仕様整理・設計 | 2 | 1.行動意思決定仕様の明文化.txt | 状態、優先順位、遷移条件の定義 | 進行中 |
| 1.仕様整理・設計 | 3 | 設定：移動についてのメモ.txt | 現行AIの移動構造の整理 | かなり整理済み |
| 1.仕様整理・設計 | 4 | 設定：field設計まとめ.txt | threat / heat / mana の設計要点 | かなり整理済み |
| 1.仕様整理・設計 | 5 | 熱量の演算シミュレーション.txt | heat の演算仕様の根拠 | 整理済み |
| 2.実装に落とす前準備 | 6 | 設定：ゲノム構造（基礎拡張）.txt | 受け皿となるパラメータ設計 | 参照資料として有効 |
| 2.実装に落とす前準備 | 7 | 2.行動パラメータ設計の更新.txt | genome / 固定値 / debug値 の切り分け | 未完成 |
| 3.実コードへの統合 | 8 | 3.行動ベクトル統合.txt | 実コードに threat / heat を差し込む設計 | 未着手寄り |
| 4.検証・可視化・地形整合 | 9 | 4.場の評価・可視化連携.txt | 場データ参照経路、ログ、可視化 | 未着手寄り |
| 4.検証・可視化・地形整合 | 10 | 5.地形・境界条件との整合.txt | 境界、崖際、狭所での破綻確認 | 未着手寄り |
| 将来フェーズ | 11 | 魔素の演算シミュレーション.txt | mana field の将来設計 | phase4以降寄り |

## 進行イメージ

```text
[仕様を固める]
README
  ↓
1.行動意思決定仕様の明文化
  ↓
設定：移動についてのメモ
設定：field設計まとめ
熱量の演算シミュレーション

[実装に渡せる形へ落とす]
  ↓
設定：ゲノム構造（基礎拡張）
  ↓
2.行動パラメータ設計の更新

[コードへ入れる]
  ↓
3.行動ベクトル統合

[確認して破綻を潰す]
  ↓
4.場の評価・可視化連携
  ↓
5.地形・境界条件との整合

[後続フェーズ]
  ↓
魔素の演算シミュレーション
```

## 現時点の全体見立て

- 仕様整理・設計: 60-70%
- 実装に落とす前準備: 40-50%
- 実コードへの統合: 20-30%
- 検証・可視化・地形整合: 10-20%

## 参照

- `../0.UI整理/資料/拡張方針.txt`
- `./設定：移動についてのメモ.txt`
- `./設定：ゲノム構造（基礎拡張）.txt`
