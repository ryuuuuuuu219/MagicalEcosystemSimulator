...
`Assets/script/Ingame/Creatures/after/organ/Core/AnimalAIInstaller.cs` は、まだ確率抽選を実装していない。
現在は `OrganPresetLibrary` が固定構成を `Ensure()` で配布する段階。

現在の `InstallDefaultOrgans()` は共通基盤を固定導入する。
草食・捕食者・上位捕食者ごとの構成は `OrganPresetLibrary` が `Ensure()` で配布する。
...
現在の固定構成は `旧behaviour維持設計.md` の「現在の Ensure 固定構成」を正本にする。
...
1. `OrganPresetLibrary` が共通・種別・相別の固定構成を `Ensure()` する。
2. `AIComponentGene` を読んで、固定構成以外の可変 organ を確率で追加する。
3. 低確率変異 organ を追加する。
4. 自動導入 organ を解決する。
5. `AIComponentGene.level` を生成し、対応 organ に反映する。
