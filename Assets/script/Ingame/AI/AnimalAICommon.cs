...

{
        ...
        float deltaTime)
    {
        ...
        if (inputMagnitude <= 0.0001f)
        {
            ...
            // AI???????????????????Y?????????
            // AlignToGround(actorTransform, terrain, inertialFacingVector.sqrMagnitude > 0.0001f ? inertialFacingVector : planarVelocity);
            return telemetry;
        }
        ...
        // AI???????????????????Y?????????
        // AlignToGround(actorTransform, terrain, rotatedForward.sqrMagnitude > 0.0001f ? rotatedForward : smoothedFacing);
        return telemetry;
    }
...

    {
        ...
        if (actorTransform.TryGetComponent<Rigidbody>(out var rb) && !rb.isKinematic)
        {
            ...
            Vector3 horizontalVelocity = displacement / safeDt;
